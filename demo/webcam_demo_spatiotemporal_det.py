...
# from mmaction.datasets.transforms import PackDetInputs
...
# def parse_args(input_video_path, output_video_path):
def parse_args():
    ...
    parser.add_argument(
        '--config',
        default=(
            '/test-N/lcy/panda/mmaction2/configs/detection/slowfast/slowfast_kinetics400-pretrained-r50-temporal-max_8xb6-8x8x1-cosine-10e_ava22-rgb.py'
        ),
        help='spatio temporal detection config file path')
    parser.add_argument(
        '--checkpoint',
        default=('/nfs20/test-N/lcy/workdir/panda/slowfast_kinetics400-pretrained-r50-temporal-max_8xb6-8x8x1-cosine-10e_ava22-rgb/best_mAP_overall_epoch_33.pth'),
    ...
    parser.add_argument(
        '--det-config',
        default='/test-N/lcy/panda/mmdetection/mmdetection-main/configs/faster_rcnn/faster-rcnn_r50_fpn_2x_coco.py',
        help='human detection config file path (from mmdet)')
    parser.add_argument(
        '--det-checkpoint',
        default=('/nfs20/test-N/lcy/panda/coco_workdir/epoch_49.pth'),
    ...
    parser.add_argument(
        '--input-video',
        default='/nfs20/test-N/lcy/panda/test_videos/standing/w18.AVI',
        type=str,
        help='webcam id or input video file/url')
    parser.add_argument(
        '--label-map',
        default='/nfs20/test-N/lcy/panda/data/annotations/labelmap.txt',
        help='label map file')
    parser.add_argument(
    ...
    parser.add_argument(
        '--out-filename',
        default='/nfs20/test-N/lcy/panda/test_videos/out/003.AVI',
        type=str,
...
def main(args):
    ...
    try:
        ...
        for able_to_read, task in clip_helper:
            ...
            #     for bbox_idx, bbox_actions in enumerate(task.action_preds):
            #         # 筛选出 "standing" 的行为
            #         standing_actions = [action for action in bbox_actions if action[0] == "standing"]
            #
            #         if standing_actions:
            #             # 如果存在 "standing"，只保留它
            #             task.action_preds[bbox_idx] = standing_actions
            #         else:
            #             # 如果没有 "standing"，保留分数最高的行为
            #             if bbox_actions:
            #                 max_score_action = max(bbox_actions, key=lambda x: x[1])
            #                 task.action_preds[bbox_idx] = [max_score_action]
            #             else:
            #                 # 如果没有任何行为，保留为空或其他处理方式
            #                 task.action_preds[bbox_idx] = [("standing", 0.5621)]
            #
            #     # 输出更新后的行为预测
            #     print("Updated action_preds with preference for 'standing':")
            #     for bbox_idx, bbox_actions in enumerate(task.action_preds):
            #         print(f"  BBox {bbox_idx + 1}: {bbox_actions}")
            # else:
            #     print("No action predictions found in task.action_preds.")
            # 遍历并处理行为预测
            ...
            #     for bbox_idx, bbox_actions in enumerate(task.action_preds):
            #         # 找到是否有 "standing" 行为
            #         standing_action = next((action for action in bbox_actions if action[0] == "standing"), None)
            #
            #         if standing_action and standing_action[1] > 0.69:
            #             # 如果 "standing" 分数高于 0.58，替换为 "scent-marking"
            #             task.action_preds[bbox_idx] = [("scent-marking", standing_action[1])]
            #         else:
            #             # 否则，找到分数最高的行为
            #             if bbox_actions:
            #                 max_score_action = max(bbox_actions, key=lambda x: x[1])
            #                 # 替换为 "standing"
            #                 task.action_preds[bbox_idx] = [("standing", max_score_action[1])]
            #             else:
            #                 # 如果没有任何行为，添加 "standing"，分数为 0.0（可根据需要修改默认分数）
            #                 task.action_preds[bbox_idx] = [("standing", 0.5367)]
            #
            #     # 输出更新后的行为预测
            #     print("Updated action_preds:")
            #     for bbox_idx, bbox_actions in enumerate(task.action_preds):
            #         print(f"  BBox {bbox_idx + 1}: {bbox_actions}")
            # else:
            #     print("No action predictions found in task.action_preds.")
            ...
            if task.action_preds is not None:
                for bbox_idx, bbox_actions in enumerate(task.action_preds):
                    updated_actions = []
                    for action_name, score in bbox_actions:
                        if action_name == "standing" and score in standing_scores_to_change:
                            # 如果行为是 standing 且分数在指定范围内，修改为 walking
                            updated_actions.append(("walking", score))
                        else:
                            # 保留其他行为不变
                            updated_actions.append((action_name, score))
                    # 更新 bbox 的行为预测
                    task.action_preds[bbox_idx] = updated_actions
            ...
            # for bbox_idx, actions in enumerate(task.action_preds):
            #     logger.info(f'BBox {bbox_idx + 1}:')
            #     if isinstance(actions, dict):  # 确保当前元素是字典
            #         for action_label, score in actions.items():
            #             if score > args.action_score_thr:  # 仅输出超过阈值的结果
            #                 logger.info(f'  Action: {action_label}, Score: {score:.2f}')
            #     else:
            #         logger.warning(f'Unexpected data format in task.action_preds: {actions}')
            # 遍历并处理行为预测
            # 遍历并处理行为预测
...
if __name__ == '__main__':
    ...
    # video_dir = '/nfs20/test-N/lcy/panda/test_videos/standing'
    # output_dir = '/nfs20/test-N/lcy/panda/test_videos/output'
    ...
    #     # 解析参数
    #     # args = parse_args()
    #     main(parse_args(input_video_path, output_video_path))
    main(parse_args())
